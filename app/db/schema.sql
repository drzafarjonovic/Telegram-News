...
-- ============================================================
--  v3.0: Aqlli yuborish, jim soatlar, dam olish jadvali,
--         shoshilinch (breaking) va onboarding
-- ============================================================

-- Yuborish oralig'ini daqiqada saqlash (30/45 daq va h.k. uchun).
-- Bo'sh (NULL) bo'lsa — eski interval_hours ishlatiladi (orqaga moslik).
ALTER TABLE schedules ADD COLUMN IF NOT EXISTS interval_minutes       INTEGER;

-- Aqlli rejim: yetarlicha muhim yangilik yig'ilsa, jadvaldan oldin yuboradi.
ALTER TABLE schedules ADD COLUMN IF NOT EXISTS smart_mode             BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE schedules ADD COLUMN IF NOT EXISTS smart_min_stories      SMALLINT NOT NULL DEFAULT 5;

-- Bo'sh digest yubormaslik (standart: yubormaydi).
ALTER TABLE schedules ADD COLUMN IF NOT EXISTS skip_empty             BOOLEAN NOT NULL DEFAULT TRUE;

-- Shoshilinch (breaking) xabarlar — importance=5 darhol yuboriladi.
ALTER TABLE schedules ADD COLUMN IF NOT EXISTS breaking_enabled       BOOLEAN NOT NULL DEFAULT TRUE;

-- Jim soatlar: shu oraliqda oddiy digest kechiktiriladi (breaking bundan mustasno).
ALTER TABLE schedules ADD COLUMN IF NOT EXISTS quiet_enabled          BOOLEAN NOT NULL DEFAULT TRUE;
ALTER TABLE schedules ADD COLUMN IF NOT EXISTS quiet_start            TEXT NOT NULL DEFAULT '23:00';
ALTER TABLE schedules ADD COLUMN IF NOT EXISTS quiet_end              TEXT NOT NULL DEFAULT '07:00';

-- Dam olish kunlari (shanba/yakshanba) uchun alohida jadval.
ALTER TABLE schedules ADD COLUMN IF NOT EXISTS weekend_enabled        BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE schedules ADD COLUMN IF NOT EXISTS weekend_mode           TEXT NOT NULL DEFAULT 'interval'; -- 'interval' | 'daily'
ALTER TABLE schedules ADD COLUMN IF NOT EXISTS weekend_interval_minutes INTEGER;
ALTER TABLE schedules ADD COLUMN IF NOT EXISTS weekend_daily_times    TEXT[] NOT NULL DEFAULT '{}';

-- Onboarding va "Hozir yuborish" cheklovi
ALTER TABLE users ADD COLUMN IF NOT EXISTS onboarded            BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE users ADD COLUMN IF NOT EXISTS last_manual_digest_at TIMESTAMPTZ;

-- Shoshilinch xabar allaqachon yuborilganini kuzatish (takrorlamaslik uchun)
CREATE TABLE IF NOT EXISTS breaking_deliveries (
    user_id  BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    story_id BIGINT NOT NULL REFERENCES stories(id) ON DELETE CASCADE,
    sent_at  TIMESTAMPTZ NOT NULL DEFAULT now(),
    PRIMARY KEY (user_id, story_id)
);
CREATE INDEX IF NOT EXISTS idx_breaking_deliv_user ON breaking_deliveries (user_id);
CREATE INDEX IF NOT EXISTS idx_breaking_deliv_sent ON breaking_deliveries (sent_at DESC);
