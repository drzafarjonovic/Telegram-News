...
class Config:
    ...
    # Mavjud kanaldagi bo'shliqni to'ldirishda bir siklda olinadigan MAKS xabar.
    # 0 => cheksiz (bo'shliq to'liq to'ldiriladi). Juda faol kanalda runaway'dan
    # saqlanmoqchi bo'lsangiz, masalan 5000 qo'ying.
    backfill_max_total: int = _get_int("BACKFILL_MAX_TOTAL", 0)

    # ---- Enterprise v2.0: media (OCR / Speech-to-Text) ----
    ocr_enabled: bool = os.getenv("OCR_ENABLED", "false").strip().lower() in ("1", "true", "yes", "on")
    ocr_langs: str = os.getenv("OCR_LANGS", "uzb+rus+eng").strip() or "uzb+rus+eng"
